...
import { RegisterInput, LoginInput, VerifyEmailQuery, ForgotPasswordInput, ResetPasswordInput, RefreshTokenInput } from '../schemas/auth.schema';
...
export const registerHandler = async (
  ...
  try {
    ...
    request.server.log.info(`[MAIL] Gửi link kích hoạt kèm Token: ${verificationToken}`);
...
export const loginHandler = async (
  ...
  try {
    ...
    if (error.message === 'AUTH_INVALID_CREDENTIALS') {
      return reply.status(400).send({ success: false, message: 'Tài khoản hoặc mật khẩu không chính xác.' });
    }
    if (error.message === 'AUTH_ACCOUNT_BANNED') {
      return reply.status(403).send({ success: false, message: 'Tài khoản của bạn đang bị khóa vĩnh viễn.' });
    }
    if (error.message === 'AUTH_ACCOUNT_LOCKED') {
      return reply.status(403).send({ success: false, message: 'Tài khoản của bạn đang bị tạm khóa.' });
    }
...
export const verifyEmailHandler = async (
  ...
  try {
    ...
    return reply.status(200).send({
      success: true,
      message: 'Tài khoản của bạn đã được xác thực thành công. Bây giờ bạn có thể đăng nhập.'
    });

  } catch (error: any) {
    if (error.message === 'AUTH_USER_NOT_FOUND') {
      return reply.status(404).send({ success: false, message: 'Tài khoản không tồn tại trên hệ thống.' });
    }
...
export const forgotPasswordHandler = async (
  ...
  try {
    ...
    request.server.log.info(`[MAIL] Link khôi phục mật khẩu Token: ${resetToken}`);
    ...
    if (error.message === 'AUTH_ACCOUNT_BANNED') {
      return reply.status(403).send({ success: false, message: 'Tài khoản đang bị khóa vĩnh viễn.' });
    }
...
export const resetPasswordHandler = async (
  ...
  try {
    ...
    if (decoded.purpose !== 'reset_password') {
      return reply.status(400).send({ success: false, message: 'Mã khôi phục không hợp lệ.' });
    }
...
export const refreshTokenHandler = async (
  ...
  try {
    ...
    try {
      ...
      return reply.status(401).send({ success: false, message: 'Phiên đăng nhập không hợp lệ.' });
    }

    // 2. Kiểm tra trạng thái và token đã bị hủy chưa (Truyền thêm refreshToken vào hàm)
    const user = await AuthService.validateUserForTokenRefresh(decoded.userId, refreshToken, context);

    // 3. Cấp Access Token mới
    const payload = { userId: user.id, email: user.email };
    const newAccessToken = request.server.jwt.sign(payload, { expiresIn: '1d' });
    ...
    if (error.message === 'AUTH_TOKEN_REVOKED') {
      return reply.status(401).send({ 
        success: false, 
        message: 'Token đã bị vô hiệu hóa do bạn đã đăng xuất trước đó. Vui lòng đăng nhập lại.' 
      });
    }

    if (error.message === 'AUTH_USER_NOT_FOUND' || error.message.startsWith('AUTH_ACCOUNT_')) {
      return reply.status(403).send({ success: false, message: 'Tài khoản không đủ điều kiện.' });
    }
...
export const logoutHandler = async (
  ...
  try {
    ...
    try {
      const decoded: { userId: number } = request.server.jwt.verify(refreshToken);
      await AuthService.logout(decoded.userId, context);
    } catch (error) {
      // Nếu token sai hoặc hết hạn sẵn rồi thì vẫn coi như logout thành công ở client
    }
