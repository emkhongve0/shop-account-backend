...
export class InventoryService {
  ...
  static async importAccounts(productId: number, rawTxtData: string) {
    const lines = rawTxtData.split(/\r?\n/).map(line => line.trim()).filter(line => line.length > 0);
   ...
   */
  static async getAllInventory(filters: { productId?: number; status?: AccountStatus }) {
   ...
   */
  static async createAccount(data: { productId: number; accountData: string; status?: AccountStatus }) {
   ...
   */
  static async updateAccount(id: number, data: { accountData?: string; status?: AccountStatus; productId?: number }) {
